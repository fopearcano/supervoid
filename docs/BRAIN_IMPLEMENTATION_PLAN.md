...
- **Phase 1 — Dedicated vLLM inference deployment (Prompt 1).**
...
- **Phase 3 — Conversation domain + identity + event outbox + state compiler (Prompt 3).** Migrations for `conversations`/`conversation_turns`/`conversation_participants`/`user_chat_identities`/`project_brain_state`/`domain_events`; the state compiler and outbox dispatcher; `/api/brain/*` conversation API. **State-version-aware:** the compiler stamps each `project_brain_state` with a monotonic version; every `conversation_turn` records the state version it consumed, so the Brain can detect staleness (turn version < current version) and recompile context. *Done when:* migrations apply on SQLite + Postgres, `manage_db check` passes, turns persist with their state version, and citations can reference a state version.
