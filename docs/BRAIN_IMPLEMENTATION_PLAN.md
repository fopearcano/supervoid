...
- **Phase 2 — SUPERVOID Brain Gateway (Prompt 2).** ✅ *Delivered — see [`ARCHITECTURE.md`](./ARCHITECTURE.md#supervoid-brain--openai-compatible-gateway).* OpenAI-compatible server in front of vLLM at **`/brain/v1`** (mounted outside the private `/api` prefix; migration `0018`): `GET /brain/v1/models`, `POST /brain/v1/chat/completions` (ordinary **and** streaming `data: [DONE]` SSE), `GET /brain/health`, and `POST /brain/v1/responses` (`501`, deferred). It resolves user / permitted projects / profile / Work-StoryWorld / conversation binding, builds the request through the `ContextAssembler`, calls the **provider** (which alone holds the vLLM key) and persists the turn (usage, request id, tool calls, retrieval refs, state versions, latency). *Divergence from the original sketch (intentional, more secure):* auth is **not** the browser JWT but a dedicated, hashed **Brain access token** (`sk-brain-…`, managed at `/api/brain-tokens`, never exposing the upstream key); the model allowlist is the served Gateway model; per-user rate + concurrency limits and graceful `502/503/504` provider errors are included. SUPERVOID extension metadata rides in an optional top-level `supervoid` object; machine-readable citations are returned there. Full OpenAI-client / curl / mocked-streaming / LibreChat-fixture compatibility tests pass and the existing backend suite is green.
...
- **Phase 4 — SUPERVOID MCP server (Prompt 4).** ✅ *Delivered — see [`MCP_SERVER.md`](./MCP_SERVER.md) + [`ARCHITECTURE.md`](./ARCHITECTURE.md#supervoid-mcp-server).* A governed Model Context Protocol server (Streamable HTTP) at `/mcp`: ~36 read / proposal / approval tools that re-run the policy service against the mapped user, authenticated by an internal service token + HMAC-signed user-context. Read tools return only authorised records; write-like tools create gated proposals; approval tools verify approval scope. No raw CRUD/SQL/shell/HTTP/filesystem.
- **Phase 5 — LibreChat deployment (Prompt 5).** ✅ *Delivered — see [`LIBRECHAT_INTEGRATION.md`](./LIBRECHAT_INTEGRATION.md).* LibreChat deployed as a **separately versioned, pinned upstream** app (`deploy/brain/docker-compose.librechat.yml` + `librechat.yaml`) — no fork, no vendored source. A `SUPERVOID Brain` custom endpoint points at the Brain Gateway `/brain/v1` (user-provided BrainAccessToken, served-alias model list, titles via the Brain); the `supervoid` MCP server is wired with dynamic user headers via the signing shim; Mongo + Meilisearch are private; LibreChat is reachable only behind the SUPERVOID reverse proxy (`/chat/`). Registration disabled after admin provisioning; strong JWT/refresh/creds secrets; file + rate limits; backups/upgrades/recovery documented; an automated config validator (`scripts/validate_librechat_config.py`, CI-tested) gates deploys. *Done when:* a SUPERVOID member signs in, links a Brain token, holds a multi-turn conversation through the Gateway, and MCP tool calls are governed — with no network path from the UI to vLLM.
